...
var ERIKNG = {
	...
	initFullHeightPanel: function() {
		var _this = this;
		var windowHeight = $(window).height();
		_this.el.fullHeight.css('min-height', windowHeight);
		
		if (!_this.mobileAndTabletcheck()) {
    ...
    initOrientationChangeHandler: function() {
        var _this = this;
        if (_this.mobileAndTabletcheck()) {
            window.addEventListener("orientationchange", function() {
                clearTimeout(_this.resizeTimer);
                _this.resizeTimer = setTimeout(_this.scrolltocurrent, 250);
            }, false);
        } else {
	...
	initScrollHandler: function() {
        ...
        $(window).scroll(function(event) {
            ...
            if (!animatingdown) {
                if ($(panelId + (currentpage)).offset().top - $(window).scrollTop() >= 80) {
